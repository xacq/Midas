...
namespace MidasD
{
    public partial class FrmDevolucionContabilidad: Form
    {
        ...
        public FrmDevolucionContabilidad(SrMidasD.Usuario usuario)
        {
            InitializeComponent();
        ...
        //public void listarFianzas()
        //{

        //    dgvLista.DataSource = servicio.pafianzaListarFuncionariosBuscarRRHH(Util.header, txtBuscar.Text, " ");
        //    Utils.Wfa.hideHeadersDGV(dgvLista, new List<string>() { "idFuncionario", "idFianza" });
        //    Utils.Wfa.positionHeadersDGV(dgvLista, new List<string>() { "numero_Memorando", "nombre_Completo", "numero_Documento", "fecha_limite_fianza", "tipo_contrato_item", "vigencia_Contrato", "cargo", "haber_Mensual", "tipo_Fianza", "estado_Fianza" });
        ...
        private async Task cargarCampos()
        {
            ...
            {
                MessageBox.Show("Ya no se puede Editar Porque el funcionario ya tiene una esta fianza devuelta.", "::: MidasD - Información :::", MessageBoxButtons.OK, MessageBoxIcon.Information);
                limpiarErrores();
                //listar();
        ...
        private async Task editar()
        {
            ...
            {
                MessageBox.Show("No se Puede Devolver porque el funcionario ya tiene esta fianza Devuelta.", "::: MidasD - Información :::", MessageBoxButtons.OK, MessageBoxIcon.Information);
                limpiarErrores();
                frmCargando.Close();
                await listarDevoluciones();
                if (dgvLista.Rows.Count == 0) Util.pnlListaActivar(false, btnPnlLista2);
