...
namespace MidasD
{
    public partial class FrmCertificadoImprimirReales : Form
    {
        ...
        public FrmCertificadoImprimirReales(SrMidasD.Usuario usuario)
        {
            InitializeComponent();
            ...
            btnBuscar,txtBuscar };
            btnPnlLista3 = new List<Control>() { btnSalir, pnlLista, btnNuevo };
            btnPnlLista2 = new List<Control>() {  btnEditar, btnImprimir, btnQuitarSeleccion,btnImprimir1 };
            btnPnlDatos = new List<Control>() {  btnCancelar,pnlDatos};
            Util.btn_Mouse(new List<PictureBox>() { btnEditar, btnBuscar, 
            btnCancelar,  btnSalir,btnNuevo,btnImprimir,btnQuitarSeleccion,btnImprimir1 });
            ...
            if (!verificarFianzaCurso())
            {
                try
                {
                    ...
                    try
                    {
                        ...
                        SrMidasD.Fianza fianzaDatos = servicio.fianzaGet(Util.header, idFianza);
                        int idFuncionario = (int)servicio.fianzaGet(Util.header, idFianza).idFuncionario;
                        ...
                        servicio.funcionarioEditar(Util.header, funcionario);
        ...
        private async void btnImprimir1_Click(object sender, EventArgs e)
        {
            ...
            if(!string.IsNullOrEmpty(fianza.resolucion_Administrativa) && fianza.fianza_Validada_Contabilidad==true)
            {
                ...
                try
                {
                    ...
                    new FrmCrCertificado(idFianza, 0, fecha, usuario).ShowDialog();
                    await listar();
                }
                catch
                {
            ...
            {
                MessageBox.Show("No se puede imprimir aun porque no ha sido validada por contabilidad.", "::: MidasD - Información :::", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            try
            {
                ...
                try
                {
                    foreach (DataGridViewRow item in dgvLista.Rows)
                    {
                        ...
                        try
                        {
                            impresa = (servicio.fianzaGet(Util.header,Convert.ToInt32(item.Cells["idFianza"].Value.ToString())).fianza_Impresa_RRHH.Value).ToString();
                        }
                        catch { }

                        try
                        {
                            asesor = (item.Cells["resolucion_Administrativa"]).Value.ToString();
                        }
                        catch { }

                        try
                        {
                            validada = (item.Cells["fianza_Validada_Contabilidad"].Value).ToString();
                        }
                        catch { }
                        ...
                        if (impresa == "True")
                        {
                            Util.pintarDatagridwiewIndex(dgvLista, index, "Black", "Lime");
                        }
                        else
                        {
                            if (!string.IsNullOrEmpty(asesor) || validada == "True")
                            {

                                if (validada == "True" && !string.IsNullOrEmpty(asesor))
                                {
                                ...
                                else
                                {
                                    ...
                                    {
                                        Util.pintarDatagridwiewIndex(dgvLista, index, "Black", "Coral");
                                    }
                                }
                            }
